...
void ViEBaseImpl::RequestKeyFrame (int param) {
  ViEChannelManagerScoped cs(*(shared_data_.channel_manager()));
  ViEEncoder* vie_encoder = cs.Encoder(param);
  vie_encoder->SendKeyFrame ();
}

void ViEBaseImpl::OnRTPNetChanged (int param, int lost) {
  ViEChannelManagerScoped cs(*(shared_data_.channel_manager()));
  ViEEncoder* vie_encoder = cs.Encoder(param);
  vie_encoder->OnRTPNetChanged (lost);
}
...
int ViEBaseImpl::StartSend(const int video_channel) {
  ...
  // 设置Rtp发送情况监控的回调
  vie_channel->Set_RtpReliableSenderCB (this, video_channel);
