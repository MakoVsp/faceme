...
ViEChannel::ViEChannel(WebRtc_Word32 channel_id,
  ...
  #if USE_VIDEO_RTP_RELIABLE
  rtp_helper_->SetMaxBufferSize (RtpReliableHelper::VIDEO_MAX_BUF);
  rtp_rtcp_->SetRtpHelper (rtp_helper_);
  default_rtp_rtcp_->SetRtpHelper (rtp_helper_);
  bandwidth_observer_->SetBlock (true);				// 禁用视频的RTCP流量调节
  #endif
}
