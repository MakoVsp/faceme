...
WebRtc_Word32 ViEEncoder::SetEncoder(const webrtc::VideoCodec& video_codec) {
  ...
  start_bitrate_ = video_codec.startBitrate * 1000;
  min_bitrate_ = video_codec.minBitrate * 1000;
  max_bitrate_ = video_codec.maxBitrate * 1000;
  return 0;
}
...
void ViEEncoder::OnRTPNetChanged (int loss) {
	// 维持当前的发送速率不变
	if (loss == 0) {
		return;
	}

	int ori_br = start_bitrate_;

	// 发生丢包，减小发送速率
	if (loss > 0) {
		start_bitrate_ = start_bitrate_ * (100 - loss) / 100;
	}
	// 负值，增大发送速率
	else {
		start_bitrate_ = start_bitrate_ * (100 - loss) / 100;
	...
	start_bitrate_  = max (start_bitrate_, min_bitrate_);
	start_bitrate_ = min (start_bitrate_, max_bitrate_);

	vcm_.SetChannelParameters(start_bitrate_ / 1000, 0, 1);
	default_rtp_rtcp_->SetTargetSendBitrate(start_bitrate_);
