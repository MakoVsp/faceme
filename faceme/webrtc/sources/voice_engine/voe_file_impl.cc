    ...
    )
{
    WEBRTC_TRACE(kTraceApiCall, kTraceVoice, VoEId(_shared->instance_id(), -1),
                 ...
                 printf("StartPlayingFileLocally(channel=%d, fileNameUTF8[]=%s, "
                 "loop=%d, format=%d, volumeScaling=%5.3f, startPointMs=%d,"
                 " stopPointMs=%d)\n",
                 channel, fileNameUTF8, loop, format, volumeScaling,
                 startPointMs, stopPointMs);
...
printf("StartPlayingFileLocally\n");
...
int VoEFileImpl::IsPlayingFileLocally(int channel)
{
    WEBRTC_TRACE(kTraceApiCall, kTraceVoice, VoEId(_shared->instance_id(), -1),
                 ...
                 printf("IsPlayingFileLocally(channel=%d)\n", channel);
    if (!_shared->statistics().Initialized())
    {
        _shared->SetLastError(VE_NOT_INITED, kTraceError);
        return -1;
...
int VoEFileImpl::ScaleLocalFilePlayout(int channel, float scale)
{
    WEBRTC_TRACE(kTraceApiCall, kTraceVoice, VoEId(_shared->instance_id(), -1),
                 ...
                 channel, scale);
                 printf("ScaleLocalFilePlayout(channel=%d, scale=%5.3f)\n",
                 channel, scale);
    if (!_shared->statistics().Initialized())
    {
        _shared->SetLastError(VE_NOT_INITED, kTraceError);
                                              ...
                                              float volumeScaling)
{
    WEBRTC_TRACE(kTraceApiCall, kTraceVoice, VoEId(_shared->instance_id(), -1),
                 ...
                 volumeScaling);
                 printf("StartPlayingFileAsMicrophone(channel=%d, fileNameUTF8=%s, "
                 "loop=%d, mixWithMicrophone=%d, format=%d, "
                 "volumeScaling=%5.3f)\n",
                 channel, fileNameUTF8, loop, mixWithMicrophone, format,
                 volumeScaling);
    ...
    else
    {
        ...
        else
        {
            channelPtr->SetMixWithMicStatus(mixWithMicrophone);
                printf("StartPlayingFileAsMicrophone() to start playing file\n");
            return 0;
        }
    }
}
    ...
    else
    {
        ...
        if (channelPtr == NULL)
        {
            ...
            return -1;
                ...
                printf("StopPlayingFileAsMicrophone() \n");
    ...
    int maxSizeBytes)
{
    WEBRTC_TRACE(kTraceApiCall, kTraceVoice, VoEId(_shared->instance_id(), -1),
                 ...
                 printf("StartRecordingPlayout(channel=%d, fileNameUTF8=%s, "
                 "compression, maxSizeBytes=%d)\n",
                 channel, fileNameUTF8, maxSizeBytes);
                 printf("VoEFileImpl::StartRecordingPlayout  fileNameUTF8=%s\n",fileNameUTF8);
    assert(1024 == FileWrapper::kMaxFileNameSize);
    ...
    else
    {
        ...
        if (channelPtr == NULL)
        {
            ...
            return -1;
                ...
                printf("StartRecordingPlayout() to locate channel\n");
    ...
    const char* fileNameUTF8, CodecInst* compression, int maxSizeBytes)
{
    WEBRTC_TRACE(kTraceApiCall, kTraceVoice, VoEId(_shared->instance_id(), -1),
                 ...
                 printf("VoEFileImpl::StartRecordingMicrophone fileNameUTF8=%s\n",fileNameUTF8);
    assert(1024 == FileWrapper::kMaxFileNameSize);
...
int VoEFileImpl::GetPlaybackPosition(int channel, int& positionMs)
{
    WEBRTC_TRACE(kTraceApiCall, kTraceVoice, VoEId(_shared->instance_id(), -1),
                 ...
                 printf("GetPlaybackPosition(channel=%d)\n", channel);
    ...
    if (channelPtr == NULL)
    {
        ...
        return -1;
            ...
            printf("GetPlaybackPosition() to locate channel\n");
