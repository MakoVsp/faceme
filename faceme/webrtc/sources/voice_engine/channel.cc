...
//                 printf("Channel::SendData(frameType=%u, payloadType=%u, timeStamp=%u,"
//                 " payloadSize=%u, fragmentation=0x%x)\n",
  //               frameType, payloadType, timeStamp, payloadSize, fragmentation);
...
Channel::IncomingDtmf(const WebRtc_UWord8 digitDtmf, const bool end)
{
    WEBRTC_TRACE(kTraceInfo, kTraceVoice, VoEId(_instanceId,_channelId),
               ...
               digitDtmf, end);
               printf("Channel::IncomingDtmf(digitDtmf=%u, end=%d)\n",
               digitDtmf, end);
                           ...
                           const WebRtc_UWord16 fromPort)
{
        ...
        if (_encryptionPtr)
        {
            ...
            if (decryptedBufferLength <= 0)
            {
                _engineStatisticsPtr->SetLastError(
                    ...
                    printf("Channel::IncomingRTPPacket() decryption failed\n");
                return;
            }
                                      ...
                                      (WebRtc_UWord16)rtpBufferLength) == -1)
    {
        _engineStatisticsPtr->SetLastError(
            ...
            printf("Channel::IncomingRTPPacket() RTP packet is invalid\n");
        return;
    }
}
                            ...
                            const WebRtc_UWord16 fromPort)
{
                                      ...
                                      (WebRtc_UWord16)rtcpBufferLength) == -1)
    {
        _engineStatisticsPtr->SetLastError(
            ...
            printf("Channel::IncomingRTPPacket() RTCP packet is invalid\n");
        return;
    }
}
                              ...
                              const WebRtc_UWord8 volume)
{
    WEBRTC_TRACE(kTraceStream, kTraceVoice, VoEId(_instanceId,_channelId),
                 ...
                 printf("Channel::OnPlayTelephoneEvent(id=%d, event=%u, lengthMs=%u,"
                 " volume=%u)\n", id, event, lengthMs, volume);
                               ...
                               const WebRtc_UWord32 SSRC)
{
    WEBRTC_TRACE(kTraceInfo, kTraceVoice, VoEId(_instanceId,_channelId),
                 ...
                 id, SSRC);
                 printf("Channel::OnIncomingSSRCChanged(id=%d, SSRC=%d)\n",
                 id, SSRC);
                                    ...
                                    const bool added)
{
    WEBRTC_TRACE(kTraceInfo, kTraceVoice, VoEId(_instanceId,_channelId),
                 ...
                 id, CSRC, added);
                 printf("Channel::OnIncomingCSRCChanged(id=%d, CSRC=%d, added=%d)\n",
                 id, CSRC, added);
...
printf("_audioCodingModule.IncomingPacket\n");
...
Channel::Channel(const WebRtc_Word32 channelId,
	...
	#if USE_AUDIO_RTP_RELIABLE
	,rtp_helper_ (RtpReliableHelper::CreateRTPReliableHelper ())
	#endif
{
	...
	#if USE_AUDIO_RTP_RELIABLE
	rtp_helper_->SetMaxBufferSize (RtpReliableHelper::AUDIO_MAX_BUF);
	_rtpRtcpModule->SetRtpHelper (rtp_helper_);
	#endif
...
Channel::~Channel()
{
	...
	#if USE_AUDIO_RTP_RELIABLE
	RtpReliableHelper::ReleaseRTPReliableHelper (rtp_helper_);
	#endif
}
...
Channel::Init()
{
    ...
    if (rtpRtcpFail)
    {
        _engineStatisticsPtr->SetLastError(
            ...
            printf("Channel::Init() RTP/RTCP module not initialized\n");
        return -1;
    }
            printf("Channel::Init() RTP/RTCP module initialized\n");
    ...
    if (fail)
    {
        _engineStatisticsPtr->SetLastError(
            ...
            printf("Channel::Init() callbacks not registered\n");
        return -1;
    }
            printf("Channel::Init() callbacks registered\n");
    ...
    for (int idx = 0; idx < nSupportedCodecs; idx++)
    {
            ...
            (_rtpRtcpModule->RegisterReceivePayload(codec) == -1))
        {
            WEBRTC_TRACE(kTraceWarning, kTraceVoice,
                         ...
                         printf("Channel::Init() unable to register %s (%d/%d/%d/%d) "
                         "to RTP/RTCP receiver\n",
                         codec.plname, codec.pltype, codec.plfreq,
                         codec.channels, codec.rate);
        }
        else
        {
            WEBRTC_TRACE(kTraceInfo, kTraceVoice,
                         ...
                         codec.plname, codec.pltype, codec.plfreq,
                         codec.channels, codec.rate);
                         printf("Channel::Init() %s (%d/%d/%d/%d) has been added to "
                         "the RTP/RTCP receiver\n",
                         codec.plname, codec.pltype, codec.plfreq,
                         codec.channels, codec.rate);
        }
        ...
        if (!STR_CASE_CMP(codec.plname, "ILBC") && (codec.channels == 1)) //change by zl  default ILBC
        {
            SetSendCodec(codec);
        }
...
Channel::StartPlayout()
{
    ...
    if (_outputMixerPtr->SetMixabilityStatus(*this, true) != 0)
    {
        _engineStatisticsPtr->SetLastError(
          ...
          //  printf("StartPlayout() failed to add participant to mixer\n");
        return -1;
    }
    ...
    if (!_externalTransport)
    {
        if (!_socketTransportModule.ReceiveSocketsInitialized())
        {
            _engineStatisticsPtr->SetLastError(
                ...
                printf("StartReceive() must set local receiver first\n");
            return -1;
        }
        if (_socketTransportModule.StartReceiving(KNumberOfSocketBuffers) != 0)
        {
            _engineStatisticsPtr->SetLastError(
                ...
                printf("StartReceiving() failed to start receiving\n");
            return -1;
        }
    }
#endif
                          ...
                          const char multicastIpAddr[64])
{
    WEBRTC_TRACE(kTraceInfo, kTraceVoice, VoEId(_instanceId,_channelId),
                 ...
                 printf("Channel::SetLocalReceiver()\n");
printf("file = %s, func = %s, line = %d \n", __FILE__, __FUNCTION__, __LINE__ );
	...
	printf("Channel::SetLocalReceiver ok\n");
    return 0;
}
#endif
...
Channel::GetLocalReceiver(int& port, int& RTCPport, char ipAddr[64])
{
    WEBRTC_TRACE(kTraceInfo, kTraceVoice, VoEId(_instanceId,_channelId),
                 ...
                 printf("Channel::GetLocalReceiver()\n");
                                                        ...
                                                        multicastIpAddr) != 0)
    {
        _engineStatisticsPtr->SetLastError(
            ...
            printf("GetLocalReceiver() unable to retrieve socket information\n");
        return -1;
    }

            printf("GetLocalReceiver() retrieve socket information, port = %d, RTCPport = %d \n", port, RTCPport );
    ...
    if (sourcePort != kVoEDefault)
    {
                                                            ...
                                                            NULL) != 0)
        {
            _engineStatisticsPtr->SetLastError(
                ...
                printf("SetSendDestination() failed to retrieve socket information\n");
            return -1;
        }
...
Channel::SetOnHoldStatus(bool enable, OnHoldModes mode)
{
    WEBRTC_TRACE(kTraceInfo, kTraceVoice, VoEId(_instanceId,_channelId),
                 ...
                 printf("Channel::SetOnHoldStatus()\n");
...
Channel::RegisterVoiceEngineObserver(VoiceEngineObserver& observer)
{
    WEBRTC_TRACE(kTraceInfo, kTraceVoice, VoEId(_instanceId,_channelId),
                 ...
                 printf("Channel::RegisterVoiceEngineObserver()\n");
    CriticalSectionScoped cs(&_callbackCritSect);
...
WebRtc_Word32 Channel::RegisterExternalTransport(Transport& transport)
{
    WEBRTC_TRACE(kTraceInfo, kTraceVoice, VoEId(_instanceId, _channelId),
               ...
               printf("Channel::RegisterExternalTransport()\n");
...
Channel::ReceivedRTPPacket(const WebRtc_Word8* data, WebRtc_Word32 length)
{
    WEBRTC_TRACE(kTraceStream, kTraceVoice, VoEId(_instanceId,_channelId),
                 ...
                 printf("Channel::ReceivedRTPPacket()\n");
...
Channel::SetSendTOS(int DSCP, int priority, bool useSetSockopt)
{
    WEBRTC_TRACE(kTraceInfo, kTraceVoice, VoEId(_instanceId,_channelId),
                 ...
                 printf("Channel::SetSendTOS(DSCP=%d, useSetSockopt=%d)\n",
                 DSCP, (int)useSetSockopt);
                                     ...
                                     const CodecInst* codecInst)
{
    WEBRTC_TRACE(kTraceInfo, kTraceVoice, VoEId(_instanceId,_channelId),
                 ...
                 printf("Channel::StartPlayingFileLocally(fileNameUTF8[]=%s, loop=%d,"
                 " format=%d, volumeScaling=%5.3f, startPosition=%d, "
                 "stopPosition=%d)\n", fileName, loop, format, volumeScaling,
                 startPosition, stopPosition);
...
int Channel::RegisterFilePlayingToMixer()
{
    ...
    if (_outputMixerPtr->SetAnonymousMixabilityStatus(*this, true) != 0)
    {
        ...
        _engineStatisticsPtr->SetLastError(
            ...
            printf("StartPlayingFile() failed to add participant as file to mixer\n");
        _outputFilePlayerPtr->StopPlayingFile();
        FilePlayer::DestroyFilePlayer(_outputFilePlayerPtr);
        _outputFilePlayerPtr = NULL;
        return -1;
	...
	printf("Channel::RegisterFilePlayingToMixer\n");
    return 0;
}
...
int Channel::ScaleLocalFilePlayout(const float scale)
{
        ...
        (_outputFilePlayerPtr->SetAudioScaling(scale) != 0))
    {
        ...
        return -1;
                 ...
                 printf("Channel::ScaleLocalFilePlayout(scale=%5.3f)\n", scale);
...
printf("Channel::StartPlayingFileAsMicrophone\n");
    return 0;
}
                                   ...
                                   const CodecInst* codecInst)
{
    WEBRTC_TRACE(kTraceInfo, kTraceVoice, VoEId(_instanceId,_channelId),
                 ...
                 printf("Channel::StartRecordingPlayout(fileName=%s)\n", fileName);
    ...
    CodecInst dummyCodec={100,"L16",16000,320,0,320000};
...
printf("end of Channel::StartRecordingPlayout\n");
    return 0;
}
    ...
    bool useForRTCP)
{
    WEBRTC_TRACE(kTraceInfo, kTraceVoice, VoEId(_instanceId,_channelId),
               "Channel::EnableSRTPReceive()");
               printf("Channel::EnableSRTPReceive()\n");
    ...
    TelephoneEventDetectionMethods& detectionMethod)
{
    WEBRTC_TRACE(kTraceInfo, kTraceVoice, VoEId(_instanceId, _channelId),
                 "Channel::GetTelephoneEventDetectionStatus()");
                 printf("Channel::GetTelephoneEventDetectionStatus()\n");
...
Channel::RegisterRTPObserver(VoERTPObserver& observer)
{
    WEBRTC_TRACE(kTraceInfo, kTraceVoice, VoEId(_instanceId, _channelId),
                 ...
                 printf("Channel::RegisterRTPObserver()\n");
    CriticalSectionScoped cs(&_callbackCritSect);
...
Channel::RegisterRTCPObserver(VoERTCPObserver& observer)
{
    WEBRTC_TRACE(kTraceInfo, kTraceVoice, VoEId(_instanceId,_channelId),
                 ...
                 printf("Channel::RegisterRTCPObserver()\n");
    CriticalSectionScoped cs(&_callbackCritSect);
    ...
    if (CSRCs > 0)
    {
        ...
        for (int i = 0; i < (int) CSRCs; i++)
        {
            WEBRTC_TRACE(kTraceStateInfo, kTraceVoice,
                       ...
                       printf("GetRemoteCSRCs() => arrCSRC[%d]=%lu\n", i, arrCSRC[i]);
        }
    } else
    {
        WEBRTC_TRACE(kTraceStateInfo, kTraceVoice,
...
Channel::GetRTCPStatus(bool& enabled)
{
    ...
    WEBRTC_TRACE(kTraceStateInfo, kTraceVoice,
                 ...
                 printf("GetRTCPStatus() => enabled=%d\n", enabled);
    return 0;
}
...
Channel::GetRemoteRTCP_CNAME(char cName[256])
{
    if (cName == NULL)
    {
        _engineStatisticsPtr->SetLastError(
            ...
            printf("GetRemoteRTCP_CNAME() invalid CNAME input buffer\n");
        return -1;
    }
    ...
    if (_rtpRtcpModule->RemoteCNAME(remoteSSRC, cname) != 0)
    {
        _engineStatisticsPtr->SetLastError(
            ...
            printf("GetRemoteRTCP_CNAME() failed to retrieve remote RTCP CNAME\n");
        return -1;
    }
    strcpy(cName, cname);
    WEBRTC_TRACE(kTraceStateInfo, kTraceVoice,
                 ...
                 printf("GetRemoteRTCP_CNAME() => cName=%s\n", cName);
    return 0;
}
    ...
    unsigned short* fractionLost)
{
    ...
    if (_rtpRtcpModule->RemoteRTCPStat(&senderInfo) != 0)
    {
        _engineStatisticsPtr->SetLastError(
            ...
            "side");
            printf("GetRemoteRTCPData() failed to retrieve sender info for remote "
            "side\n");
        return -1;
    }
            printf("GetRemoteRTCPData() to retrieve sender info for remote \n");
    ...
    if (NULL != jitter || NULL != fractionLost)
    {
        ...
        if (_rtpRtcpModule->RemoteRTCPStat(&remote_stats) != 0 ||
          ...
          WEBRTC_TRACE(kTraceWarning, kTraceVoice,
                       ...
                       printf("GetRemoteRTCPData() failed to measure statistics due"
                       " to lack of received RTP and/or RTCP packets\n");
          return -1;
        }
          ...
          *fractionLost = it->fractionLost;
          WEBRTC_TRACE(kTraceStateInfo, kTraceVoice,
                       ...
                       *fractionLost);
                       ...
                       printf("GetRemoteRTCPData() => fractionLost = %lu\n",
                       *fractionLost);
    }
    return 0;
}
...
int Channel::GetRemoteRTCPSenderInfo(SenderInfo* sender_info) {
  if (sender_info == NULL) {
    _engineStatisticsPtr->SetLastError(VE_INVALID_ARGUMENT, kTraceError,
        ...
        printf("GetRemoteRTCPSenderInfo() invalid sender_info.\n");
    return -1;
  }
  ...
  if (_rtpRtcpModule->RemoteRTCPStat(&rtcp_sender_info) != 0) {
    _engineStatisticsPtr->SetLastError(VE_RTP_RTCP_MODULE_ERROR, kTraceError,
        ...
        printf("GetRemoteRTCPSenderInfo() failed to read RTCP SR sender info.\n");
    return -1;
  }
                              ...
                              unsigned short payloadSize)
{
    WEBRTC_TRACE(kTraceInfo, kTraceVoice, VoEId(_instanceId, _channelId),
               ...
               printf("Channel::InsertExtraRTPPacket()\n");
    if (payloadType > 127)
    {
        _engineStatisticsPtr->SetLastError(
    ...
    VoEMediaProcess& processObject)
{
    WEBRTC_TRACE(kTraceInfo, kTraceVoice, VoEId(_instanceId,_channelId),
                 ...
                 printf("Channel::RegisterExternalMediaProcessing()\n");
	...
	printf("_outputExternalMediaCallbackPtr = %d, _outputExternalMedia = %d\n", _outputExternalMediaCallbackPtr, _outputExternalMedia);
    return 0;
}
...
printf("Channel::MixOrReplaceAudioWithFile   _inputFilePlayerPtr->Get10msAudioFromFile\n");
...
printf("Channel::MixAudioWithFile   _outputFilePlayerPtr->Get10msAudioFromFile\n");
...
Channel::SendPacketRaw(const void *data, int len, bool RTCP)
{
    ...
    if (!RTCP)
    {
			printf("Channel::SendPacketRaw rtp \n");
