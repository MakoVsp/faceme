...
//printf("VoEBaseImpl::RecordedDataIsAvailable\n");
    return 0;
}
...
int VoEBaseImpl::RegisterVoiceEngineObserver(VoiceEngineObserver& observer)
{
    WEBRTC_TRACE(kTraceApiCall, kTraceVoice, VoEId(_shared->instance_id(), -1),
                 ...
                 printf("RegisterVoiceEngineObserver(observer=0x%d)\n", &observer);
    CriticalSectionScoped cs(&_callbackCritSect);
    if (_voiceEngineObserverPtr)
    {
...
int VoEBaseImpl::Init(AudioDeviceModule* external_adm)
{
    ...
    if (_shared->process_thread())
    {
        if (_shared->process_thread()->Start() != 0)
        {
            _shared->SetLastError(VE_THREAD_ERROR, kTraceError,
                ...
                //printf("Init() failed to start module process thread, startVal = %d\n", _shared->process_thread()->Start());
                printf("Init() failed to start module process thread");
            return -1;
        }
    }
    ...
    if (external_adm == NULL)
    {
        ...
        if (_shared->audio_device() == NULL)
        {
            _shared->SetLastError(VE_NO_MEMORY, kTraceCritical,
                ...
                printf("Init() failed to create the ADM\n");
            return -1;
        }
    }
    else
    {
        ...
        WEBRTC_TRACE(kTraceInfo, kTraceVoice, VoEId(_shared->instance_id(), -1),
            ...
            printf("An external ADM implementation will be used in VoiceEngine\n");
    }
    ...
    if (_shared->audio_device()->RegisterEventObserver(this) != 0) {
      _shared->SetLastError(VE_AUDIO_DEVICE_MODULE_ERROR, kTraceWarning,
          ...
          printf("Init() failed to register event observer for the ADM\n");
    }
    ...
    if (_shared->audio_device()->RegisterAudioCallback(this) != 0) {
      _shared->SetLastError(VE_AUDIO_DEVICE_MODULE_ERROR, kTraceWarning,
          ...
          printf("Init() failed to register audio callback for the ADM\n");
    }
    ...
    if (_shared->audio_device()->Init() != 0)
    {
        _shared->SetLastError(VE_AUDIO_DEVICE_MODULE_ERROR, kTraceError,
            ...
            printf("Init() failed to initialize the ADM\n");
        return -1;
    }
            printf("Init() success to initialize the ADM\n");
            ...
            WEBRTC_VOICE_ENGINE_DEFAULT_DEVICE) != 0)
    {
        _shared->SetLastError(VE_AUDIO_DEVICE_MODULE_ERROR, kTraceInfo,
            ...
            printf("Init() failed to set the default output device\n");
    }
    if (_shared->audio_device()->SpeakerIsAvailable(&available) != 0)
    {
        _shared->SetLastError(VE_CANNOT_ACCESS_SPEAKER_VOL, kTraceInfo,
            ...
            printf("Init() failed to check speaker availability, trying to initialize speaker anyway\n");
    }
    else if (!available)
    {
        _shared->SetLastError(VE_CANNOT_ACCESS_SPEAKER_VOL, kTraceInfo,
            ...
            "anyway");
            printf("Init() speaker not available, trying to initialize speaker anyway\n");
    }
    if (_shared->audio_device()->InitSpeaker() != 0)
    {
        _shared->SetLastError(VE_CANNOT_ACCESS_SPEAKER_VOL, kTraceInfo,
            "Init() failed to initialize the speaker");
            printf("Init() failed to initialize the speaker\n");
    }
            ...
            WEBRTC_VOICE_ENGINE_DEFAULT_DEVICE) != 0)
    {
        _shared->SetLastError(VE_SOUNDCARD_ERROR, kTraceInfo,
            ...
            printf("Init() failed to set the default input device\n");
    }
    if (_shared->audio_device()->MicrophoneIsAvailable(&available) != 0)
    {
        _shared->SetLastError(VE_CANNOT_ACCESS_MIC_VOL, kTraceInfo,
            ...
            printf("Init() failed to check microphone availability, trying to initialize microphone anyway\n");
    }
    else if (!available)
    {
        _shared->SetLastError(VE_CANNOT_ACCESS_MIC_VOL, kTraceInfo,
            ...
            "microphone anyway");
            printf("Init() microphone not available, trying to initialize microphone anyway\n");
    }
    if (_shared->audio_device()->InitMicrophone() != 0)
    {
        _shared->SetLastError(VE_CANNOT_ACCESS_MIC_VOL, kTraceInfo,
            "Init() failed to initialize the microphone");
            printf("Init() failed to initialize the microphone\n");
    }
    ...
    if (_shared->audio_device()->StereoPlayoutIsAvailable(&available) != 0) {
      _shared->SetLastError(VE_SOUNDCARD_ERROR, kTraceWarning,
          ...
          printf("Init() failed to query stereo playout mode\n");
    }
    if (_shared->audio_device()->SetStereoPlayout(available) != 0)
    {
        _shared->SetLastError(VE_SOUNDCARD_ERROR, kTraceWarning,
            "Init() failed to set mono/stereo playout mode");
            printf("Init() failed to set mono/stereo playout mode\n");
    }
    ...
    if (_shared->audio_device()->SetStereoRecording(available) != 0)
    {
        _shared->SetLastError(VE_SOUNDCARD_ERROR, kTraceWarning,
            ...
            printf("Init() failed to set mono/stereo recording mode\n");
    }
    ...
    if (_shared->audio_processing() == NULL)
    {
        ...
        if (_shared->audio_processing() == NULL)
        {
            _shared->SetLastError(VE_NO_MEMORY, kTraceCritical,
                ...
                printf("Init() failed to create the AP module\n");
            return -1;
        }
                        ...
                        kVoiceEngineAudioProcessingDeviceSampleRateHz))
        {
            _shared->SetLastError(VE_APM_ERROR, kTraceError,
                ...
                " module");
                printf("Init() failed to set the device sample rate to 48K for AP module\n");
            return -1;
        }
        ...
        if (_shared->audio_processing()->set_sample_rate_hz(8000))
        {
            _shared->SetLastError(VE_APM_ERROR, kTraceError,
                ...
                printf("Init() failed to set the sample rate to 8K for AP module\n");
            return -1;
        }
        ...
        if (_shared->audio_processing()->set_num_channels(1, 1) != 0)
        {
            _shared->SetLastError(VE_SOUNDCARD_ERROR, kTraceError,
                ...
                printf("Init() failed to set channels for the primary audio stream\n");
            return -1;
        }
                ...
                WEBRTC_VOICE_ENGINE_HP_DEFAULT_STATE) != 0)
        {
            _shared->SetLastError(VE_APM_ERROR, kTraceError,
                ...
                printf("Init() failed to set the high-pass filter for AP module\n");
            return -1;
        }
        // Echo Cancellation
        if (_shared->audio_processing()->echo_cancellation()->
                enable_drift_compensation(false) != 0)
        {
            _shared->SetLastError(VE_APM_ERROR, kTraceError,
                ...
                printf("Init() failed to set drift compensation for AP module\n");
            return -1;
        }
                ...
                WEBRTC_VOICE_ENGINE_EC_DEFAULT_STATE))
        {
            _shared->SetLastError(VE_APM_ERROR, kTraceError,
                ...
                printf("Init() failed to set echo cancellation state for AP module\n");
            return -1;
        }
        // Noise Reduction
                ...
                != 0)
        {
            _shared->SetLastError(VE_APM_ERROR, kTraceError,
                ...
                printf("Init() failed to set noise reduction level for AP module\n");
            return -1;
        }
                ...
                WEBRTC_VOICE_ENGINE_NS_DEFAULT_STATE) != 0)
        {
            _shared->SetLastError(VE_APM_ERROR, kTraceError,
                ...
                printf("Init() failed to set noise reduction state for AP module\n");
            return -1;
        }
                ...
                set_analog_level_limits(kMinVolumeLevel,kMaxVolumeLevel) != 0)
        {
            _shared->SetLastError(VE_APM_ERROR, kTraceError,
                ...
                printf("Init() failed to set AGC analog level for AP module\n");
            return -1;
        }
                ...
                != 0)
        {
            _shared->SetLastError(VE_APM_ERROR, kTraceError,
                ...
                printf("Init() failed to set AGC mode for AP module\n");
            return -1;
        }
                ...
                != 0)
        {
            _shared->SetLastError(VE_APM_ERROR, kTraceError,
                ...
                printf("Init() failed to set AGC state for AP module\n");
            return -1;
        }
        // VAD
                ...
                != 0)
        {
            _shared->SetLastError(VE_APM_ERROR, kTraceError,
                ...
                printf("Init() failed to set VAD state for AP module\n");
            return -1;
        }
    }
            ...
            != GainControl::kFixedDigital)
    {
        ...
        if (_shared->audio_device()->SetAGC(enable) != 0)
        {
            _shared->SetLastError(VE_AUDIO_DEVICE_MODULE_ERROR,
                ...
                printf("Init() failed to set default AGC mode in ADM 0\n");
        }
    }
#endif
...
printf("bf channelPtr->SetLocalReceiver\n");
    return channelPtr->SetLocalReceiver(port, rtcpPortUW16, ipAddr,
                                        multiCastAddr);
printf("af channelPtr->SetLocalReceiver\n");
#else
...
int VoEBaseImpl::SetNetEQPlayoutMode(int channel, NetEqModes mode)
{
    WEBRTC_TRACE(kTraceApiCall, kTraceVoice, VoEId(_shared->instance_id(), -1),
                 ...
                 printf("SetNetEQPlayoutMode(channel=%i, mode=%i)\n", channel, mode);
    if (!_shared->statistics().Initialized())
    {
        _shared->SetLastError(VE_NOT_INITED, kTraceError);
        return -1;
...
//	printf("file = %s, func = %s, line = %d \n", __FILE__, __FUNCTION__, __LINE__ );
 ...
 //               printf("StartPlayout() failed to initialize playout\n");
            return -1;
        }
        if (_shared->audio_device()->StartPlayout() != 0)
        {
            WEBRTC_TRACE(kTraceError, kTraceVoice,
                VoEId(_shared->instance_id(), -1),
                "StartPlayout() failed to start playout");
   //             printf("StartPlayout() failed to start playout\n");
            return -1;
        }
    //            printf("StartPlayout() to start playout\n");
    }
    return 0;
}
