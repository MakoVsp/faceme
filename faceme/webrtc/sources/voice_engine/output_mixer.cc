                                       ...
                                       const CodecInst* codecInst)
{
    WEBRTC_TRACE(kTraceInfo, kTraceVoice, VoEId(_instanceId,-1),
                 ...
                 printf("OutputMixer::StartRecordingPlayout(fileName=%s)\n", fileName);
...
//printf("OutputMixer::DoOperationsOnCombinedSignal _externalMedia = %d,  _externalMediaCallbackPtr = %d \n", _externalMedia, _externalMediaCallbackPtr);
    if (_externalMedia)
    {
        CriticalSectionScoped cs(&_callbackCritSect);
...
OutputMixer::InsertInbandDtmfTone()
{
    ...
    if (_audioFrame.num_channels_ == 1)
    {
			printf("_audioFrame.data_,mono\n");
        // mono
        memcpy(_audioFrame.data_, toneBuffer, sizeof(WebRtc_Word16)
            * toneSamples);
    } else
    {
			printf("_audioFrame.data_,stereo\n");
        // stereo
