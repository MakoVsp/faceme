...
//add by zl 2015 6 30
WebRtc_Word32 AudioMixerManagerLinuxPulse::SetLoudspeakerStatus(int enable)
{
    WEBRTC_TRACE(kTraceInfo, kTraceAudioDevice, _id,
                 "AudioMixerManagerLinuxPulse::SetLoudspeakerStatus(enable=%u)",
                 enable);
    ...
    if(true)
    {
        ...
        if(mpAudioManager){
            ...
            if(enable == 0){
                ...
                WEBRTC_LOG("setPort--------------------------------------------------AM_PORT_OUTPUT_IHF");
                mpAudioManager->setPort(AudioManager::AM_PORT_OUTPUT_IHF);//AM_PORT_OUTPUT_IHF 表示外放

            }
            else if(enable == 3 ){

                WEBRTC_LOG("setPort--------------------------------------------------AM_PORT_CALLON_HEADPHONE");
                mpAudioManager->setPort(AudioManager::AM_PORT_CALLON_HEADPHONE);//AM_PORT_CALLON_EARPIECE 表示耳机

            }  else if(enable == 5 ){

                WEBRTC_LOG("setPort--------------------------------------------------AM_PORT_OUTPUT_IHFHEADPHONE");
                mpAudioManager->setPort(AudioManager::AM_PORT_OUTPUT_IHFHEADPHONE);//AM_PORT_OUTPUT_IHFHEADPHONE 表示耳机加外放

            } else {
                WEBRTC_LOG("setPort--------------------------------------------------AM_PORT_CALLON_HEADPHONES");
                mpAudioManager->setPort(AudioManager::AM_PORT_CALLON_EARPIECE);//AM_PORT_CALLON_EARPIECE 表示听筒
    ...
    } else
    {
        ...
        _paSpeakerLoud = enable;
    }

    if (setFailed)
    {
        WEBRTC_TRACE(kTraceWarning, kTraceAudioDevice, _id,
                     " could not loud speaker, error%d",
...
bool AudioMixerManagerLinuxPulse::GetSinkInputInfo() const {
    ...
    printf("AudioMixerManagerLinuxPulse::GetSinkInputInfo\n");
    return true;
}
