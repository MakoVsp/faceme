...
WebRtc_Word32 AudioDeviceLinuxPulse::Init()
{
    ...
    if (InitPulseAudio() < 0)
    {
        ...
        if (TerminatePulseAudio() < 0)
        {
            ...
            printf("  failed to terminate PulseAudio\n");
        }
    ...
    if (_ptrThreadRec == NULL)
    {
        ...
        printf("  failed to create the rec audio thread\n");
        return -1;
    }

    unsigned int threadID(0);
    int ret = _ptrThreadRec->Start(threadID);
    //if (!_ptrThreadRec->Start(threadID))
    if (!ret)
    {
        WEBRTC_TRACE(kTraceCritical, kTraceAudioDevice, _id,
                     "  failed to start the rec audio thread");
        printf("  failed to start the rec audio threadID = %d\n", threadID );
        printf("  failed to start the rec audio threadID ret = %d\n", ret );

        delete _ptrThreadRec;
        _ptrThreadRec = NULL;
        return -1;
    }
    printf("  successfully to start the rec audio threadID ret = %d\n", ret );
    ...
    if (_ptrThreadPlay == NULL)
    {
        ...
        printf(" failed to create the play audio thread\n");
        return -1;
    }
    ...
    if (!_ptrThreadPlay->Start(threadID))
    {
        ...
        printf("  failed to start the play audio thread\n");
    ...
    printf("  successfully to start the play audio thread\n");
...
{
    WEBRTC_LOG("AudioDeviceLinuxPulse------------------Terminate");
    if (!_initialized)
    {
        return 0;
    }
...
WebRtc_Word32 AudioDeviceLinuxPulse::InitSpeaker()
{
    ...
    _deviceIndex = -1;
    _paDeviceIndex = -1;
    printf("AudioDeviceLinuxPulse::InitSpeaker success!\n");
    return 0;
}
...
//add by zl 2015 6 30
WebRtc_Word32 AudioDeviceLinuxPulse::SetLoudspeakerStatus(int enable)
{
    WEBRTC_TRACE(kTraceInfo, kTraceAudioDevice, _id,
                 "AudioDeviceLinuxPulse::SetLoudspeakerStatus(enable=%d)", enable);
...
WebRtc_Word32 AudioDeviceLinuxPulse::SetPlayoutDevice(WebRtc_UWord16 index)
{
    ...
    printf("  number of availiable output devices is %u\n", nDevices);
    ...
    printf("  device index is of range [0,%u]\n", (nDevices - 1));

    _outputDeviceIndex = index;
    _outputDeviceIsSpecified = true;

    printf("  _outputDeviceIndex = %d \n", index);
    printf("  _outputDeviceIsSpecified = %d \n", _outputDeviceIsSpecified);
    return 0;
}
        ...
        char guid[kAdmMaxGuidSize])
{
    ...
    printf("  _playDisplayDeviceName = %s \n", name);
    printf("  _deviceIndex = %d \n", index);
    // get playout devices
    PlayoutDevices();
...
WebRtc_Word32 AudioDeviceLinuxPulse::InitPlayout()
{
    ...
    printf("AudioDeviceLinuxPulse::InitPlayout successfully!\n");
    return 0;
}
        ...
        WebRtc_UWord16 sizeMS)
{
    ...
    printf( "set _playBufType = %d, _playBufDelayFixed = %d\n", type, sizeMS );
    return 0;
}
                                                      ...
                                                      int eol)
{
    ...
    if (_numPlayDevices == _deviceIndex)
    {
        ...
        if (_playDeviceName)
        {
            ...
            printf( "_playDeviceName = [%s], line = %d, function = %s \n", _playDeviceName, __LINE__, __FUNCTION__ );
        }
...
void AudioDeviceLinuxPulse::PaStreamStateCallbackHandler(pa_stream *p)
{
    ...
    case PA_STREAM_UNCONNECTED:
        WEBRTC_TRACE(kTraceDebug, kTraceAudioDevice, _id,
                     "  unconnected");
        printf( "stream state =  unconnected.\n " );
        break;
    case PA_STREAM_CREATING:
        WEBRTC_TRACE(kTraceDebug, kTraceAudioDevice, _id,
                     "  creating");
        printf( "stream state =  creating.\n " );
        break;
    case PA_STREAM_FAILED:
    case PA_STREAM_TERMINATED:
        WEBRTC_TRACE(kTraceDebug, kTraceAudioDevice, _id,
                     "  failed");
        printf( "stream state =  failed.\n " );
        break;
    case PA_STREAM_READY:
        WEBRTC_TRACE(kTraceDebug, kTraceAudioDevice, _id,
                     "  ready");
        printf( "stream state =  ready.\n " );
        break;
    }
...
WebRtc_Word32 AudioDeviceLinuxPulse::CheckPulseAudioVersion()
{
    ...
    printf("  checking PulseAudio version: %s\n", _paServerVersion);
...
{
    int retVal = 0;
    WEBRTC_LOG("AudioDeviceLinuxPulse-----InitPulseAudio----------------------------1");
    // Load libpulse
    ...
    WEBRTC_LOG("AudioDeviceLinuxPulse-----InitPulseAudio----------------------------2");
    ...
    if (!_paMainloop)
    {
        ...
        printf("  could not create mainloop\n");
        return -1;
    }
    printf("  create mainloop\n");
    ...
    if (retVal != PA_OK)
    {
        ...
        printf("  failed to start main loop, error=%d\n", retVal);
        return -1;
    }
    ...
    if (_paContext){
        ...
        printf("  PA context has already existed\n");
        PaUnLock();
        return -1;
    }
    ...
    if (retVal != PA_OK)
    {
        ...
        PaUnLock();
        printf("  failed to connect context, error=%d\n", retVal);
        return -1;
    }
    ...
    if (CheckPulseAudioVersion() < 0)
    {
        ...
        printf("  PulseAudio version %s not supported\n", _paServerVersion);
        return -1;
    }
    ...
    if (InitSamplingFrequency() < 0 || _samplingFreq == 0)
    {
        WEBRTC_TRACE(kTraceError, kTraceAudioDevice, _id,
                     ...
                     _samplingFreq);
        printf("  failed to initialize sampling frequency, set to %d\n",
               _samplingFreq);
        return -1;
    }
    printf("_mixerManager.SetPulseAudioObjects\n");
    return 0;
}
...
{
    WEBRTC_LOG("AudioDeviceLinuxPulse----------------------------PaStreamWriteCallbackHandler");
...
{
    WEBRTC_LOG("AudioDeviceLinuxPulse------------------PaStreamReadCallbackHandler");
                                                      ...
                                                      size_t bufferSize)
{
    ...
    if (_recordBufferUsed > 0)
    {
        ...
        //printf( "ProcessRecordedData, &_recBuffer = %s\n", &_recBuffer );
        _recordBufferUsed = 0;
    }
    ...
    {
        //			printf( "file:%s,line:%d,Provide playout data to VoiceEngine:bufferData = %s \n", bufferData, __FILE__, __LINE__ );
        // Provide data to VoiceEngine
    ...
    //printf("after ProcessRecordedData\n");
...
bool AudioDeviceLinuxPulse::PlayThreadProcess()
{
    ...
    case kEventTimeout:
        WEBRTC_TRACE(kTraceWarning, kTraceAudioDevice, _id,
                     "EventWrapper::Wait() timeout");
        return true;
    }
    Lock();

    WEBRTC_TRACE(kTraceWarning, kTraceAudioDevice, _id,
                 " after EventWrapper::Wait()，after lock ");

    if (_startPlay)
    {
        WEBRTC_TRACE(kTraceInfo, kTraceAudioDevice, _id,
                     "_startPlay true, performing initial actions");
        printf("_startPlay true, performing initial actions\n");
        ...
        if(_paContext == NULL ||_playStream == NULL ){
            UnLock();
             PaUnLock();
            return false;
        }
                    ...
                    ptr_cvolume, NULL) != PA_OK)
        {
            WEBRTC_TRACE(kTraceError, kTraceAudioDevice, _id,
                         ...
                         LATE(pa_context_errno)(_paContext));
            printf("  failed to connect play stream, err=%d\n",
                   LATE(pa_context_errno)(_paContext));
        }
        ...
        {
             if(!PA_STREAM_IS_GOOD(LATE(pa_stream_get_state)(_playStream) )) {
    ...
    //printf("after PlayThreadProcess\n");
    UnLock();
    return true;
}
...
bool AudioDeviceLinuxPulse::RecThreadProcess()
{
    ...
    if (_startRec)
    {
        ...
        while (LATE(pa_stream_get_state)(_recStream) != PA_STREAM_READY)
        {
            ...
            if(!PA_STREAM_IS_GOOD(LATE(pa_stream_get_state)(_recStream) )) {
    ...
    if (_recording)
    {
        //			printf( "recording data and provide to VoiceEngine \n" );
