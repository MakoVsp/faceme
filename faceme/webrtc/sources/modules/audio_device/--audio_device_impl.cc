...
printf("before CreatePlatformSpecificObjects\n");
...
printf("before AttachAudioBuffer\n");
...
printf("before return audioDevice\n");
    return audioDevice;
}
...
printf("audioLayer = %d\n", audioLayer);
    ...
    if ((audioLayer == kLinuxPulseAudio) || (audioLayer == kPlatformDefaultAudio))
    {
        ...
        printf("attempting to use the Linux PulseAudio APIs...\n");
        ...
        if (AudioDeviceLinuxPulse::PulseAudioIsSupported())
        {
            ...
            printf("Linux PulseAudio APIs will be utilized\n");
        }
        else
        {
#endif
...
printf("_platformAudioLayer = %d\n", _platformAudioLayer);
            if (ptrAudioDevice != NULL)
            {
                ...
                printf("Linux PulseAudio is *not* supported => ALSA APIs will be utilized instead\n");
            }
#endif
#if defined(LINUX_PULSE)
        }
    ...
    else if (audioLayer == kLinuxAlsaAudio)
    {
        ...
        printf("Linux ALSA APIs will be utilized\n");
#endif
    }
    ...
    if (audioLayer == kDummyAudio)
    {
        ...
        ptrAudioDevice = new AudioDeviceDummy(Id());
        WEBRTC_TRACE(kTraceInfo, kTraceAudioDevice, _id, "Dummy Audio APIs will be utilized");
        printf("Dummy Audio APIs will be utilized\n");
    ...
    if (ptrAudioDevice == NULL)
    {
        ...
        printf("unable to create the platform specific audio device implementation\n");
        return -1;
    }
    ...
    if (ptrAudioDeviceUtility == NULL)
    {
        ...
        printf("unable to create the platform specific audio device utility\n");
        return -1;
    }
...
{
printf("AudioDeviceModuleImpl::Init()\n");
    if (_initialized)
        return 0;

    if (!_ptrAudioDeviceUtility)
{printf("_ptrAudioDeviceUtility\n");
        return -1;
}
    if (!_ptrAudioDevice)
{printf("_ptrAudioDevice\n");
        return -1;
}
    WebRtcSpl_Init();
    ...
    {
//printf("_ptrAudioDevice->Init()=%d\n",_ptrAudioDevice->Init());
printf("_ptrAudioDevice->Init()= -1\n");
        return -1;
    }
...
WebRtc_Word16 AudioDeviceModuleImpl::PlayoutDevices()
{
    ...
    printf("output: #playout devices=%d\n", nPlayoutDevices);
...
WebRtc_Word16 AudioDeviceModuleImpl::RecordingDevices()
{
    ...
    WEBRTC_TRACE(kTraceStateInfo, kTraceAudioDevice, _id,
                 ...
                 printf("output: #recording devices=%d\n", nRecordingDevices);
...
{
    CHECK_INITIALIZED_BOOL();
	printf( "AudioDeviceModuleImpl::Playing = %d, %s \n", _ptrAudioDevice->Playing(), _ptrAudioDevice->Playing() );
...
printf("AudioDeviceModuleImpl::StartRawInputFileRecording\n");
...
printf("file = %s, func = %s, line = %d \n", __FILE__, __FUNCTION__, __LINE__ );
    ...
    if (_ptrAudioDevice->PlayoutDelay(delay) == -1)
    {
        ...
        printf("failed to retrieve the playout delay\n");
        return -1;
    }
