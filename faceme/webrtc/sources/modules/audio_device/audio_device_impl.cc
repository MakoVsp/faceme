...
WebRtc_Word32 AudioDeviceModuleImpl::CreatePlatformSpecificObjects()
{
    ...
    if ((audioLayer == kLinuxPulseAudio) || (audioLayer == kPlatformDefaultAudio))
    {
        ...
        WEBRTC_LOG("ZL----------------------------------AudioDeviceLinuxPulse::PulseAudioIsSupported---1");
        if (AudioDeviceLinuxPulse::PulseAudioIsSupported())
        {
            WEBRTC_LOG("ZL----------------------------------AudioDeviceLinuxPulse::PulseAudioIsSupported---2");
            // create *Linux PulseAudio* implementation
            ptrAudioDevice = new AudioDeviceLinuxPulse(Id());
            WEBRTC_LOG("ZL----------------------------------AudioDeviceLinuxPulse::PulseAudioIsSupported---3");
...
WebRtc_Word32 AudioDeviceModuleImpl::SetLoudspeakerStatus(int enable)
{
    CHECK_INITIALIZED();
