...
extern"C" void ndk_log (const char* l);
...
WebRtc_Word32 VideoRenderOpenGles20::Setup(WebRtc_Word32 width,
  ...
  // 当前所知道的不支持NPOT的设备列表
  const char* POT_Devices[] = {
	  "Adreno 200",
	  "Adreno 205",
  };

  mPOT = false;
  for (int i=0; i<sizeof (POT_Devices) / sizeof (const char*); i++) {
	  if (strcasecmp (POT_Devices[i], device) == 0) {
		  mPOT = true;
		  break;
	  }
  }
  ...
  // 因为 POT 中，纹理拉大了，所以对应的贴图位置缩小
  if (mPOT) {
	  _vertices [8] = _vertices [13] = 352 / 512.0f;
	  _vertices [14] = _vertices [19] = 288 / 512.0f;
  }
...
WebRtc_Word32 VideoRenderOpenGles20::Render(const VideoFrame& frameToRender) {
  ...
  // 少数情况，不支持NPOT
	  ...
	  CKPotFrame (frameToRender);
	  if (_textureWidth != (GLsizei) mPOTFrame.Width() ||
		  _textureHeight != (GLsizei) mPOTFrame.Height()) {
		SetupTextures(mPOTFrame);
	  }
	  else {
		UpdateTextures(mPOTFrame);
	  }
  }
...
void VideoRenderOpenGles20::CKPotFrame (const VideoFrame& frameToRender) {
  ...
  int newSize = pot_width * pot_height * 3 / 2;
  bool noInit = mPOTFrame.Size () <= 0;
  mPOTFrame.VerifyAndAllocate (newSize);
  if (noInit) {
	  memset (mPOTFrame.Buffer (), 0, sizeof (newSize));
  }

  unsigned char* dst = mPOTFrame.Buffer ();
  unsigned char* src = frameToRender.Buffer ();

  int i;
  for (i=0; i<height; i++) {
	  memcpy (dst + i * pot_width, src + i * width, width);
  }

  int uHeight = height / 2;
  int uWidth = width / 2;
  int uPotHeight = pot_height / 2;
  int uPotWidth = pot_width / 2;

  dst = dst + pot_width * pot_height;
  src = src + width * height;

  unsigned char* dstv = dst + uPotHeight * uPotWidth;
  unsigned char* srcv = src + uHeight * uWidth;

  for (i=0; i<uHeight; i++) {
	  memcpy (dst + uPotWidth * i, src + uWidth * i, uWidth);
	  memcpy (dstv + uPotWidth * i, srcv + uWidth * i, uWidth);
  }

  mPOTFrame.SetLength (newSize);
  mPOTFrame.SetWidth (pot_width);
  mPOTFrame.SetHeight (pot_height);
  mPOTFrame.SetTimeStamp (frameToRender.TimeStamp ());
  mPOTFrame.SetRenderTime (frameToRender.RenderTimeMs ());
}
