...
WebRtc_Word32 RTPSender::SendPaddingAccordingToBitrate(
    ...
    //		printf("RTPSender::SendPaddingAccordingToBitrate\n");
...
WebRtc_Word32 RTPSender::ReSendToNetwork(const WebRtc_UWord8* packet,
    ...
    //		printf("RTPSender::ReSendToNetwork\n");
...
RTPSender::OnReceivedNACK(const WebRtc_UWord16 nackSequenceNumbersLength,
    ...
    printf("RTPSender::OnReceivedNACK\n");
...
void RTPSender::ProcessSendToNetwork() {
    //		printf("RTPSender::ProcessSendToNetwork\n");
...
WebRtc_Word32
RTPSender::SendToNetwork2(WebRtc_UWord8* buffer,
                          ...
                          bool key_start)
{
    ...
    bool reset = false;
    unsigned int cur_sequence = _sequenceNumber - 1;		// 当前包的sequence
    //printf("RTPSender::SendToNetwork2\n");
    //printf("file = %s, func = %s, line = %d \n", __FILE__, __FUNCTION__, __LINE__ );
    // 重发没有收到响应的包
    if (m_rtp_helper != NULL) {
        m_rtp_helper->Lock (true);		// 锁定对象数据
        if (m_rtp_helper->PopResendPack (&data, &size, &header_size, &timestamp)) {
            BuildRTPHeaderExtension2 ( (WebRtc_UWord8 *) (data + 12), false);		// 设置当前的sequence rtp标准头12字节，在此基础上，添加扩展头
            SendToNetwork ( (WebRtc_UWord8*) data, size - header_size, header_size, -1, kDontRetransmit);					// size 整个包的大小
        }

        // 如果当前发送缓冲已满，必须抛弃
        if (!m_rtp_helper->is_allow_send (key_start, &cur_sequence, &reset, capture_time_ms)) {
            allow_send = false;
        }
        m_rtp_helper->Lock (false);		// 释放锁
    }
    ...
    // 查看是否sequence发生变化 （从发送锁定状态到恢复状态，会造成sequence变化，恢复为锁定前的sequence）
    if (cur_sequence != _sequenceNumber - 1) {
        _sequenceNumber = cur_sequence;
        ModuleRTPUtility::AssignUWord16ToBuffer(buffer+2, _sequenceNumber);
        sequence = _sequenceNumber;
        _sequenceNumber++;
        m_rtp_helper->log ("\r\n ____________________ restore sequence %u", _sequenceNumber);
    }

    //printf("file = %s, func = %s, line = %d \n", __FILE__, __FUNCTION__, __LINE__ );
    // 把当前的包推入网络
    BuildRTPHeaderExtension2 ( (WebRtc_UWord8 *) (buffer + 12), reset);		// 设置当前的sequence rtp标准头12字节，在此基础上，添加扩展头
    WebRtc_Word32 ret = SendToNetwork (buffer, length, rtpLength, -1, storage);

    //printf("file = %s, func = %s, line = %d \n", __FILE__, __FUNCTION__, __LINE__ );
    // 把当前发过的包推入等待响应队列
    if (m_rtp_helper != NULL) {
        m_rtp_helper->SendOutgoingPack ( (const char*) buffer, length + rtpLength, rtpLength, capture_time_ms, sequence);	// length 没有包括rtp头的长度
    }
...
RTPSender::SendToNetwork(WebRtc_UWord8* buffer,
                         ...
                         const StorageType storage)
{
    //		printf("RTPSender::SendToNetwork\n");
                          ...
                          const bool incSequenceNumber)
{
    ...
    snprintf (debug_infor, sizeof (debug_infor), "BuildRTPheader payloadType %d", payloadType);
    //ndk_log (debug_infor);
    WEBRTC_LOG(debug_infor);
    WEBRTC_LOG("----------------------------------------------------------------------------------------------");
...
{
    //printf( "RTPSender::BuildRTPHeaderExtension\n" );
    ...
    */
    ...
    while (type != kRtpExtensionNone)
    {
        ...
        else if (type == kRtpExtensionAck
                 || type == kRtpExtensionAck2) {
            unsigned int ack = -1;				// 设置为空的
            block_length = BuildRtpACKExtension (dataBuffer + kHeaderLength + total_block_length, type, ack);
        }
        else if (type == kRtpReset) {
            unsigned int reset = 0;				// 设置为空的
            block_length = BuildRtpReset (dataBuffer + kHeaderLength + total_block_length, type, reset);
        }
...
WebRtc_UWord16
RTPSender::BuildRTPHeaderExtension2(WebRtc_UWord8* dataBuffer, bool reset) const
{
    //printf( "RTPSender::BuildRTPHeaderExtension2\n" );
    if (_rtpHeaderExtensionMap.Size() <= 0) {
        return 0;
    }
    ...
    */
    ...
    while (type != kRtpExtensionNone)
    {
        ...
        if (type == kRtpExtensionTransmissionTimeOffset)
        {
            ...
            unsigned int ack = -1;							// 设置为实际的sequence，如果存在的话
            if (m_rtp_helper != NULL) {
                m_rtp_helper->PopACKTimestamp ( (unsigned long *) &ack);
            }
            block_length = BuildRtpACKExtension (dataBuffer + kHeaderLength + total_block_length, type, ack);
        }
        else if (type == kRtpExtensionAck2) {
            unsigned int ack = -1;							// 设置为实际的sequence，如果存在的话
            if (m_rtp_helper != NULL) {
                m_rtp_helper->PopACKTimestamp2 ( (unsigned long *) &ack);
            }
            block_length = BuildRtpACKExtension (dataBuffer + kHeaderLength + total_block_length, type, ack);
        }
        else if (type == kRtpReset) {
            unsigned int value_reset = reset ? 1 : 0;							// 设置为实际的sequence，如果存在的话
            block_length = BuildRtpReset (dataBuffer + kHeaderLength + total_block_length, type, value_reset);
        }
        total_block_length += block_length;
        type = _rtpHeaderExtensionMap.Next(type);
    }
...
{
    //printf( "RTPSender::BuildTransmissionTimeOffsetExtension\n" );
...
WebRtc_UWord8
RTPSender::BuildRtpACKExtension(WebRtc_UWord8* dataBuffer, RTPExtensionType type, unsigned int ack) const
{
    //		printf("RTPSender::BuildRtpACKExtension\n");
    // From RFC 5450: Transmission Time Offsets in RTP Streams.
    //
    // The transmission time is signaled to the receiver in-band using the
    // general mechanism for RTP header extensions [RFC5285]. The payload
    ...
    // Get id defined by user.
    WebRtc_UWord8 id;
    if (_rtpHeaderExtensionMap.GetId(type, &id)
            != 0) {
        // Not registered.
        return 0;
    }
    ...
    ModuleRTPUtility::AssignUWord24ToBuffer(dataBuffer + pos,
                                            ack);
    pos += 3;
    assert(pos == RTP_ACK_LENGTH_IN_BYTES);
    return RTP_ACK_LENGTH_IN_BYTES;
}

WebRtc_UWord8
RTPSender::BuildRtpReset(WebRtc_UWord8* dataBuffer, RTPExtensionType type, unsigned int reset) const
{
    //		printf("RTPSender::BuildRtpReset\n");
    // From RFC 5450: Transmission Time Offsets in RTP Streams.
    //
    // The transmission time is signaled to the receiver in-band using the
    // general mechanism for RTP header extensions [RFC5285]. The payload
...
void RTPSender::UpdateTransmissionTimeOffset(
    ...
    //		printf("RTPSender::UpdateTransmissionTimeOffset\n");
...
{
    //		printf("RTPSender::RegisterSendTransport\n");
    CriticalSectionScoped cs(_transportCritsect);
...
{
    //		printf("RTPSender::SetSendingStatus\n");
    if(enabled)
    {
        WebRtc_UWord32 freq;
        if(_audioConfigured)
...
{
    //		printf("RTPSender::RegisterAudioCallback\n");
    if(!_audioConfigured)
    {
        return -1;
    }
...
{
    printf("RTPSender::SendTelephoneEvent\n");
    if(!_audioConfigured)
    {
        return -1;
    }
...
{
    printf("RTPSender::SendTelephoneEventActive\n");
    if(!_audioConfigured)
    {
        return false;
    }
