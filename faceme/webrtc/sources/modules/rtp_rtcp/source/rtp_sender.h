...
public:
	...
	// 对发送到网络的函数扩展，增加缓存的机制
	virtual WebRtc_Word32 SendToNetwork2 (WebRtc_UWord8* dataBuffer,
                                        const WebRtc_UWord16 payloadLength,
                                        const WebRtc_UWord16 rtpHeaderLength,
                                        int64_t capture_time_ms,
										...
										bool key_start) = 0;
	virtual WebRtc_Word32 GetRTPSequence () = 0;
};
...
public:
    ...
    */
	...
	// 实际加入sequence
    WebRtc_UWord16 BuildRTPHeaderExtension2(WebRtc_UWord8* dataBuffer, bool reset) const;

    WebRtc_UWord8 BuildTransmissionTimeOffsetExtension(
        WebRtc_UWord8* dataBuffer) const;

	WebRtc_UWord8 BuildRtpACKExtension(WebRtc_UWord8* dataBuffer, RTPExtensionType type, unsigned int ack) const;

	WebRtc_UWord8 BuildRtpReset(WebRtc_UWord8* dataBuffer, RTPExtensionType type, unsigned int ack) const;
    ...
    */
	...
	// 返回当前的sequence
	WebRtc_Word32 GetRTPSequence () { return _sequenceNumber; };
    ...
    virtual WebRtc_Word32 SendToNetwork(WebRtc_UWord8* dataBuffer,
                                        ...
                                        const StorageType storage);
	// 对发送到网络的函数扩展，增加缓存的机制
	WebRtc_Word32 SendToNetwork2 (WebRtc_UWord8* dataBuffer,
                                        const WebRtc_UWord16 payloadLength,
                                        const WebRtc_UWord16 rtpHeaderLength,
                                        int64_t capture_time_ms,
                                        const StorageType storage,
    ...
    */
	...
	void SetRtpHelper (RtpReliableHelper* p) { m_rtp_helper = p; }
...
private:
	...
	RtpReliableHelper		  *m_rtp_helper;
};
} // namespace webrtc
