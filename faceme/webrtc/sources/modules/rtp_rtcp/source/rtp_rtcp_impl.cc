...
WebRtc_Word32 ModuleRtpRtcpImpl::IncomingPacket(
  ...
  WEBRTC_TRACE(kTraceStream,
               ...
               printf("IncomingPacket(packetLength:%u)",
               incomingPacketLength);
  ...
  if (rtpParser.RTCP()) {
	...
	// 当前的包可能是重复的，或者是不连续的，需要缓存或者丢弃
	if (m_rtp_helper != NULL
		&& !m_rtp_helper->RecvPack ( (const char*) incomingPacket, incomingPacketLength, rtpHeader.header.timestamp, rtpHeader.header.sequenceNumber, 
		rtpHeader.extension.rtpACK, rtpHeader.extension.rtpACK2, rtpHeader.extension.rtpReset)) {
		try_read_buffered_frames ();
	}
	// 当前的包是连续的，直接处理
	else {
		try_read_buffered_frames ();		
...
void ModuleRtpRtcpImpl::SetRtpHelper (RtpReliableHelper* p) { 
	m_rtp_helper = p; 
	_rtpSender.SetRtpHelper (m_rtp_helper); 
	RegisterSendRtpHeaderExtension (kRtpExtensionAck, 2);			// 注册一个ack的扩展
	RegisterReceiveRtpHeaderExtension (kRtpExtensionAck, 2);		// 注册一个ack的扩展
	RegisterSendRtpHeaderExtension (kRtpExtensionAck2, 3);			// 注册一个ack的扩展
	RegisterReceiveRtpHeaderExtension (kRtpExtensionAck2, 3);		// 注册一个ack的扩展
	RegisterSendRtpHeaderExtension (kRtpReset, 4);			// 注册一个ack的扩展
	RegisterReceiveRtpHeaderExtension (kRtpReset, 4);		// 注册一个ack的扩展
}

void ModuleRtpRtcpImpl::try_read_buffered_frames () {
	char* data;
	int size;

	while (m_rtp_helper != NULL
		&& m_rtp_helper->PopBufferedPack (&data, &size)) {
		// 解析该包
		ModuleRTPUtility::RTPHeaderParser rtpParser ( (const WebRtc_UWord8*) data, size);
		WebRtcRTPHeader rtpHeader;
		memset (&rtpHeader, 0, sizeof(rtpHeader));
		...
		if (rtpParser.Parse(rtpHeader, &map)) {
			_rtpReceiver.IncomingRTPPacket (&rtpHeader, (const WebRtc_UWord8*) data, size);
		}
...
//    printf("SendOutgoingData(frameType:%d payloadType:%d timeStamp:%u size:%u)\n",
  //  frameType, payloadType, timeStamp, payloadSize);
