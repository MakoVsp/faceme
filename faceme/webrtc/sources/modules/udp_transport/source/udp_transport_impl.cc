...
{
		printf("UdpTransportImpl::SetToS\n");
    if (_qos)
    {
...
UdpTransportImpl::ErrorCode UdpTransportImpl::BindRTPSendSocket()
{
    ...
    if(_ipV6Enabled)
    {
		...
		printf( "ipv4 _localRTPAddr._sockaddr_in.sin_port = %d, _localRTPAddr._sockaddr_in.sin_addr = %s \n", _localRTPAddr._sockaddr_in.sin_port, _localRTPAddr._sockaddr_in.sin_addr );
    }
...
UdpTransportImpl::ErrorCode UdpTransportImpl::BindRTCPSendSocket()
{
    ...
    if(_ipV6Enabled)
    {
		...
		printf( "ipv4 _localRTCPAddr._sockaddr_in.sin_port = %d, _localRTCPAddr._sockaddr_in.sin_addr = %s \n", _localRTCPAddr._sockaddr_in.sin_port, _localRTCPAddr._sockaddr_in.sin_addr );
    }
    return kNoSocketError;
}
...
printf( "rec _localIP = %s, _localPort = %d \n", _localIP, _localPort );
...
#endif
{
    ...
    if(_ptrRtpSocket)
    {
...
#endif
        {
            WEBRTC_TRACE(kTraceError, kTraceTransport, _id,
                         ...
                         printf("Failed to start receive on RTP socket\n");
            _lastError = kStartReceiveError;
            return -1;
        }
                         printf("start receive on RTP socket\n");
    }
    if(_ptrRtcpSocket)
    {
        if(!_ptrRtcpSocket->StartReceiving())
        {
            WEBRTC_TRACE(kTraceError, kTraceTransport, _id,
                         ...
                         printf("Failed to start receive on RTCP socket\n");
            _lastError = kStartReceiveError;
            return -1;
        }
                         printf("start receive on RTCP socket\n");
    }
        ...
        _ptrRtcpSocket == NULL)
    {
        WEBRTC_TRACE(kTraceError, kTraceTransport, _id,
                     ...
                     printf("Failed to StartReceiving, no socket initialized\n");
        _lastError = kStartReceiveError;
        return -1;
    }
    _receiving = true;
                     printf("StartReceiving, socket initialized\n");
    return 0;
}
                                       ...
                                       SocketAddress& remoteAddr) const
{
    ...
    } else
    {
        ...
        remoteAddr._sockaddr_in.sin_addr= InetAddrIPV4(
            const_cast<char*>(ip));
printf( "remoteAddr._sockaddr_in.sin_port = %d\n", remoteAddr._sockaddr_in.sin_port );
    }
}
...
{
		printf("UdpTransportImpl::IncomingRTPCallback\n");
...
void UdpTransportImpl::IncomingRTCPCallback(CallbackObj obj,
                                            ...
                                            const SocketAddress* from)
{
		printf("UdpTransportImpl::IncomingRTCPCallback\n");
                                           ...
                                           const SocketAddress* fromSocket)
{
    ...
    WebRtc_UWord16 portNr = 0;
            printf("Incoming RTP packet from ip:%s port:%d rtpPacket=%s\n", ipAddress, portNr,rtpPacket);
    {
        CriticalSectionScoped cs(_critFilter);
                                            ...
                                            const SocketAddress* fromSocket)
{
    ...
    WebRtc_UWord32 ipAddressLength = kIpAddressVersion6Length;
    WebRtc_UWord16 portNr = 0;
printf("UdpTransportImpl::IncomingRTCPFunction\n");
    {
        CriticalSectionScoped cs(_critFilter);
        if (FilterIPAddress(fromSocket) == false)
        {
