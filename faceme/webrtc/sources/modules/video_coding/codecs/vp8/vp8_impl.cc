...
int VP8EncoderImpl::InitEncode(const VideoCodec* inst,
  ...
  if (feedback_mode_) {
    ...
    config_->kf_max_dist = codec_.maxFramerate / 2;
  }
...
int VP8EncoderImpl::Encode(const VideoFrame& input_image,
  ...
  WEBRTC_TRACE(kTraceMemory, kTraceRtpRtcp, -1, "pre encode");
  ...
  WEBRTC_TRACE(kTraceMemory, kTraceRtpRtcp, -1, "after encode");
...
int VP8DecoderImpl::Decode(const EncodedImage& input_image,
  ...
  if (!feedback_mode_) {
    ...
    //if (propagation_cnt_ >= 0)
	  ...
	  if (propagation_cnt_ >= 0) {
		  char ctmp[64];
		  sprintf(ctmp, "\r\n drop %d type %d", (int) input_image._timeStamp, (int) input_image._frameType);
//		  ndk_log (ctmp);
...
#ifdef INDEPENDENT_PARTITIONS
  if (DecodePartitions(input_image, fragmentation)) {
    // Reset to avoid requesting key frames too often.
    if (propagation_cnt_ > 0) {
      propagation_cnt_ = 0;
    }
