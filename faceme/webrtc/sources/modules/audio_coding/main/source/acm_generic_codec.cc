    ...
    bool                  forceInitialization)
{
    ...
    if((_codecID >= 0) && (_codecID != codecNumber) && (_codecID != mirrorID))
    {
        ...
        WEBRTC_TRACE(webrtc::kTraceError, webrtc::kTraceAudioCoding, _uniqueID,
			"InitEncoderSafe: current codec is not the same as the one given by codecParams _codecID:%d codecNumber: %d mirrorID: %d", 
			(int) _codecID, (int) codecNumber, (int) mirrorID);
        return -1;
    }
    ...
    bool                  forceInitialization)
{
    ...
    if((_codecID >= 0) && (_codecID != codecNumber) && (_codecID != mirrorID))
    {
        WEBRTC_TRACE(webrtc::kTraceError, webrtc::kTraceAudioCoding, _uniqueID,
					...
					"by codecParams _codecID: %d codecNumber:%d mirrorID:%d", (int)_codecID, (int)codecNumber, (int)mirrorID);
        // The current codec is not the same as the one given by codecParams
        return -1;
    }
