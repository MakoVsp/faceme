...
ACMGenericCodec* ACMCodecDB::CreateCodecInstance(const CodecInst* codec_inst) {
  // All we have support for right now.
    ...
    WEBRTC_LOG ("CreateCodecInstance----------------------------------------------------------");
    char debug_code[128] = { 0 };
    snprintf(debug_code, sizeof (debug_code), "codec.pltype: %d,codec.plname :%s: codec.plfreq :%d,codec.pacsize :%d ,codec.channels :%d,codec.rate :%d", codec_inst->pltype,codec_inst->plname,codec_inst->plfreq,codec_inst->pacsize,codec_inst->channels,codec_inst->rate);
    WEBRTC_LOG (debug_code);
